...
* Zwei separate Netzwerke für Hochverfügbarkeit. Stellen Sie sicher, dass Sie Ihren iSCSI-Datenverkehr mithilfe von VLANs oder zwei separaten Netzwerken von anderen Datenverkehrsarten isolieren.
...
.Über diese Aufgabe
Diese Aufgabe beschreibt, wie Sie die iSCSI-Portkonfiguration über die Hardware-Seite aufrufen. Sie können die Konfiguration auch über das Menü:Settings[System > iSCSI-Ports konfigurieren] aufrufen.
...
. Wählen Sie im linken Bereich Menü:Hardware[Controllers and components].
