...
. Wenn sich das Dialogfeld „Selbstsignierte Zertifikate auflösen“ öffnet, überprüfen Sie die angezeigten Informationen zu den nicht vertrauenswürdigen Zertifikaten. Für weitere Informationen können Sie auch auf die Auslassungspunkte am Ende der Tabelle klicken und im Pop-up-Menü *Ansicht* auswählen.
. Führen Sie einen der folgenden Schritte aus:
+
