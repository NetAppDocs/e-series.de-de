...
.Über diese Aufgabe
Wenn Sie einen defekten HIC ersetzen, müssen Sie einen Controller offline nehmen, um das System in einen nicht-redundanten Zustand zu versetzen, den HIC ersetzen und den Controller wieder online schalten.

.Bevor Sie beginnen
* Prüfen link:hics-overview-supertask-concept.html["Anforderungen für EF300- oder EF600 HIC-Upgrades"].
* Planen Sie ein Wartungsfenster für diesen Vorgang ein. Sie können erst wieder auf die Daten des Speichersystems zugreifen, nachdem Sie diesen Vorgang erfolgreich abgeschlossen haben. Da beide Controller beim Einschalten dieselbe HIC-Konfiguration aufweisen müssen, müssen Sie einen Controller offline nehmen, um das System in einen nicht-redundanten Zustand zu versetzen, wenn Sie HICs installieren.
