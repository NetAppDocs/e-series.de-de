...
Die Befehlszeilenschnittstelle (CLI) ist eine Softwareapplikation, die eine Möglichkeit bietet, Speicher-Arrays zu konfigurieren und zu überwachen. Mit der CLI können Sie Befehle von einer Betriebssystem-Eingabeaufforderung aus ausführen, wie zum Beispiel der  `DOS C:`-Eingabeaufforderung, einem Linux-Betriebssystempfad oder einem Solaris-Betriebssystempfad.
