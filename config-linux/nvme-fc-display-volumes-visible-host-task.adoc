...
---
= Anzeige der für den Host sichtbaren Volumes in der E-Series – Linux (NVMe over FC)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Um Informationen über jeden NVMe-Pfad zu einem E-Series Volume anzuzeigen, verwenden Sie den `nvme netapp smdevices [-o <format>]` Befehl.

Der Ausgabe `<format>` Kann normal (Standard, wenn -o nicht verwendet wird), Spalte oder json sein.
