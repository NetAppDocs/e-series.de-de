...
.Über diese Aufgabe
SLES 12 verwendet Device Mapper Multipath (DMMP) für Multipathing für NVMe über RoCE. RHEL 8, RHEL 9, RHEL 10, SLES 15 und SLES 16 verwenden ein integriertes natives NVMe-Failover. Je nachdem, welches Betriebssystem Sie verwenden, ist eine zusätzliche Konfiguration von Multipath erforderlich, damit es ordnungsgemäß funktioniert.
...
NOTE: Bei SLES 15, SLES 16, RHEL 9 und RHEL 10 ist Native NVMe Multipathing standardmäßig aktiviert und es ist keine zusätzliche Konfiguration erforderlich.
