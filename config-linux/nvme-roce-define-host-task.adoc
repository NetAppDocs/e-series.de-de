...
Host-Betriebssystem-Typ
 a| 
Wählen Sie die folgende Option aus der Dropdown-Liste aus:
...
Nachdem Sie den Host erstellt haben, link:nvme-roce-assign-volume-task.html["Weisen Sie ein Volume über SANtricity System Manager zu."].
