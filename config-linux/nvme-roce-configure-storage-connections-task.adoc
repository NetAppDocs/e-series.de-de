...
.Schritte
. Wählen Sie im linken Bereich der System Manager-Oberfläche *Hardware* aus.
...
Nachdem Sie die NVMe over RoCE-Porteinstellungen konfiguriert haben, link:nvme-roce-discover-connect-storage-host-task.html["Speicher auf dem Host ermitteln und verbinden"].
